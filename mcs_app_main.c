...
static const attCfg_t mcsAppAttCfg = {
    150000, /* ATT server service discovery connection idle timeout in seconds */
    241, /* desired ATT MTU */
...
static void mcsAppSetup(dmEvt_t *pMsg)
{
    ...
    AppAdvSetData(APP_ADV_DATA_CONNECTABLE, sizeof(mcsAppAdvDataDisc),
                  (uint8_t *)mcsAppAdvDataDisc);
...
void McsAppHandlerInit(wsfHandlerId_t handlerId)
{
    ...
    APP_TRACE_INFO6("MAC Addr: %02x:%02x:%02x:%02x:%02x:%02x", addr[5], addr[4], addr[3], addr[2],
                    addr[1], addr[0]);
    APP_TRACE_INFO1("Adv local name: %s", &mcsAppScanDataDisc[2]);
